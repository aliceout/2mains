...
// Si la config SMTP est incomplète, on log un warning et on installe un
// transport "log-only" qui imprime les mails sur stdout — ça évite de
// casser le boot du CMS si le déploiement n'a pas encore les secrets.
...
export function buildEmailAdapter() {
  ...
  // Nom affiché dans le From: des mails. Hardcodé : il ne bougera pas
  // dans la durée de vie du projet, et l'externaliser en var d'env
  // ajoutait du bruit (Infisical, compose, .env.example) sans gain.
  const FROM_NAME = '2mains de femmes';
